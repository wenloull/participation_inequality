...
FACTOR_COLORS = {
    ...
    'Research_Capacity': '#CC6699',  # lavender
        ...
        if target_status == 'Over_Performing' and panel_label == 'B':
            if len(target_data) > 0:
                ...
                for idx, row in target_data.iterrows():
                    iso3 = row['ISO3']
                    disease = row['Disease']
                    if (iso3, disease) in specified_annotations:
                        ...
                        ax.annotate(combo_name,
                                    ...
                                    bbox=dict(boxstyle='round,pad=0.3',
                                              ...
                                              edgecolor='darkred',
                                              linewidth=0.5))
                        print(f"   Annotated Specified: {combo_name} (CIS={row['CIS_Country']:.3f}, PBR={row['PBR']:.3f})")
...
def visualize_network_evolution_grouped(ax, evolution_data, panel_label):
    ...
    # Combine the legends of both ax and ax2 into a single legend at upper center
    handles1, labels1 = ax.get_legend_handles_labels()
    handles2, labels2 = ax2.get_legend_handles_labels()
    ax.legend(handles1 + handles2, labels1 + labels2,
              loc='upper center', fontsize=9, framealpha=0.9, ncol=2)
...
def visualize_combined_waterfall_chart(ax, scenario1, scenario2, panel_label):
    ...
    ax.legend(by_label.values(), by_label.keys(),
              fontsize=9, framealpha=0.9, loc='upper center', ncol=2)
